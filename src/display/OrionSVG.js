...
// Extend
cv.OrionPixel.prototype = Object.create(cv.orion.Orion.prototype);
cv.OrionPixel.constructor = cv.OrionPixel;
cv.OrionPixel.constructor.name = 'cv.OrionPixel';
...
cv.OrionPixel.prototype.render = function(e) {
	...
	//context.clearRect(0, 0, this._bitmapData.width, this._bitmapData.height);
	context.globalAlpha = 0.05;           // fade rate
	context.globalCompositeOperation = "destination-out";  // fade out destination pixels
	context.fillRect(0,0,this._bitmapData.width, this._bitmapData.height);
	...
	do {
		if (particle.active) {
			...
			x = ((particle.x + 0.5)|0), y = ((particle.y + 0.5)|0); // bitshift Math.round
